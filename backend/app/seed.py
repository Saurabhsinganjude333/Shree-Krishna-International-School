...
Safe to run multiple times (idempotent) — used both as a standalone script and
automatically on backend startup (see app/main.py).
...
def run_seed():
    ...
    try:
        print("Seeding admin...")
        get_or_create_user("Rakesh Patel", "admin@skis.edu.in", "Admin@123", "admin", "9999900001")
        ...
        db.rollback()
        print(f"Seed skipped/failed safely: {e}")
    finally:
        db.close()
